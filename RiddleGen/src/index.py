...
riddle_repository.create(Riddle("A box without hinges, key, or a lid, yet golden treasure inside is hid.", "An egg")) # pylint: disable=line-too-long
...
riddle_repository.create(Riddle("What has to be broken before you can use it?", "An egg")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("I’m tall when I’m young, and I’m short when I’m old. What am I?", "A candle")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("What is always in front of you but can’t be seen?", "The future")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("What gets wet while drying?", "A towel")) # pylint: disable=line-too-long
...
riddle_repository.create(Riddle("I follow you all the time and copy your every move, but you can’t touch me or catch me. What am I?", "A shadow")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("What gets bigger when more is taken away?", "A hole")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("It belongs to you, but other people use it more than you do. What is it?", "Your name")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("What has a head and a tail but no body?", "A coin")) # pylint: disable=line-too-long
riddle_repository.create(Riddle("What building has the most stories?", "The library")) # pylint: disable=line-too-long
